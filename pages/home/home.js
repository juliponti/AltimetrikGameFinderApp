...
let card = (page) =>
  page.map((result) => {
    ...
    const card1 = ` <div class="home__main__card">
    ...
    const card2 = `<div class="one-card-view__card">
...
async function fetchData(pageNum) {
  ...
  if (threeViewVal) {
    oneViewContainer.style.display = "none";
    cardContainer.style.display = "grid";

    cardContainer.innerHTML += allCards;
    oneViewContainer.innerHTML = "";
  } else {
    oneViewContainer.style.display = "grid";
...
function handleChange(e) {
  ...
  if (
    ...
    gallery.addEventListener("scroll", handleScroll, true);
    fetchData(+1);
  } else if (currentValue != "" || e.keyCode == 13) {
    const game = currentValue.toLowerCase();
    searches.push(game);
